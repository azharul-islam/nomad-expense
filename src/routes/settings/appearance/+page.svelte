	...
	<div class="flex-1 overflow-y-auto overscroll-contain px-4 pt-4 pb-4">
		...
		<div class="overflow-hidden rounded-2xl bg-white shadow-sm dark:bg-slate-800/60">
			{#each options as option, i}
				<button
					class="flex w-full items-center justify-between px-4 py-3.5 text-left transition-colors active:bg-gray-50 dark:active:bg-slate-700 {i !==
					options.length - 1
				...
				>
					...
					{#if themeStore.preference === option.value}
						<svg
							class="h-5 w-5 text-blue-600 dark:text-sky-400"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
