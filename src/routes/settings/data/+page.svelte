<script lang="ts">
	...
	import { exportToCSV, exportToBackup, downloadCSV, downloadJSON } from '$lib/utils/export';
	...
	async function handleImport(data: ParsedImportData, mode: 'replace' | 'append') {
		const hasLoans = Boolean(data.people?.length || data.loans?.length || data.payments?.length);
		const hasTransactions = data.transactions.length > 0 || data.cards.length > 0;

		// "Replace All" means replace everything: wipe all five stores first so the file's
		// contents (whatever subsets it carries) become the whole database.
		if (mode === 'replace') {
			await loansStore.clearAll();
			await transactionStore.importData([], [], 'replace');
		}
		if (hasTransactions) {
			await transactionStore.importData(data.transactions, data.cards, mode);
		}
		if (hasLoans) {
			await loansStore.importData(
				{ people: data.people ?? [], loans: data.loans ?? [], payments: data.payments ?? [] },
				mode
			);
		}
		// Report only what the file actually carried, so a people-only backup does not
		// read as "Imported 0 transactions, 0 loans".
		const parts: string[] = [];
		if (data.summary.transactionCount > 0)
			parts.push(`${data.summary.transactionCount} transactions`);
		if (data.summary.cardCount > 0) parts.push(`${data.summary.cardCount} cards`);
		if (data.summary.personCount > 0) parts.push(`${data.summary.personCount} people`);
		if (data.summary.loanCount > 0) parts.push(`${data.summary.loanCount} loans`);
		if (data.summary.paymentCount > 0) parts.push(`${data.summary.paymentCount} loan payments`);
		showNotification(
			'success',
	...
	async function handleExportBackup() {
		if (!transactionStore.initialized) await transactionStore.init();
		if (!loansStore.initialized) await loansStore.init();
		if (transactionStore.transactions.length === 0 && !loansStore.hasAny) {
			showNotification('error', 'Nothing to back up yet');
			return;
		}
		exporting = true;
		try {
			const backup = exportToBackup({
				version: 2,
				exportedAt: Date.now(),
				transactions: transactionStore.transactions,
				cards: transactionStore.cards,
				people: loansStore.people,
				loans: loansStore.loans,
				payments: loansStore.payments
			});
			const date = new Date().toISOString().split('T')[0];
			downloadJSON(backup, 'expense-tracker-backup-' + date + '.json');
...
>
	<div class="flex-1 overflow-y-auto overscroll-contain px-4 pt-4 pb-4">
		...
		<!-- Export Section -->
		<div class="mb-6">
			<p class="mb-2 px-3 text-xs font-medium text-gray-400 uppercase dark:text-slate-400">
				Export
			</p>
			<div class="overflow-hidden rounded-2xl bg-white shadow-sm dark:bg-slate-800/60">
				<button
					class="flex w-full items-center justify-between px-4 py-3.5 text-left transition-colors active:bg-gray-50 dark:active:bg-slate-700 {transactionStore
				...
				>
					<div class="flex items-center gap-3">
						<div
							class="flex h-8 w-8 items-center justify-center rounded-lg bg-blue-50 dark:bg-sky-900/30"
						>
							<svg
								class="h-4 w-4 text-blue-600 dark:text-sky-400"
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24"
							>
			...
			<div class="overflow-hidden rounded-2xl bg-white shadow-sm dark:bg-slate-800/60">
				...
				>
					<div class="flex items-center gap-3">
						...
						<div>
							<span class="text-base text-gray-900 dark:text-white">Export full backup (JSON)</span>
							<p class="text-xs text-gray-500 dark:text-slate-400">
								Transactions, cards, people, loans and payments
							</p>
						</div>
					</div>
					<svg
			...
			<div class="overflow-hidden rounded-2xl bg-white shadow-sm dark:bg-slate-800/60">
				...
				>
					<div class="flex items-center gap-3">
						...
						<div>
							<span class="text-base text-gray-900 dark:text-white">Import CSV or backup</span>
							<p class="text-xs text-gray-500 dark:text-slate-400">
								Accepts legacy CSV or the full JSON backup
							</p>
						</div>
					</div>
					<svg
		...
		<div class="mb-6">
			...
			<div
				class="overflow-hidden rounded-2xl border border-rose-200 bg-white shadow-sm dark:border-rose-900/50 dark:bg-slate-800/60"
			>
				<button
				...
				>
					<div class="flex items-center gap-3">
						<div
							class="flex h-8 w-8 items-center justify-center rounded-lg bg-rose-50 dark:bg-rose-900/30"
						>
							<svg
						...
						<div>
							...
							<p class="text-xs text-rose-500/70 dark:text-rose-400/70">
								Permanently delete all transactions and cards
							</p>
						</div>
					</div>
	...
	{#if showImportModal}
		<ImportModal onClose={() => (showImportModal = false)} onImport={handleImport} />
	{/if}
	...
	{#if showClearConfirm}
		...
		>
			<div
				class="w-full max-w-sm rounded-t-2xl bg-white p-5 sm:rounded-2xl dark:bg-slate-800"
				role="dialog"
				aria-modal="true"
			...
			>
				...
				<p class="mb-4 text-sm text-gray-600 dark:text-slate-300">
					This permanently deletes all {transactionStore.transactions.length} transactions, cards, people
					and loans. This action cannot be undone.
				</p>
